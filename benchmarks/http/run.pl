...
say 'Benchmark::Web HTTP cross-server comparison';
...
if (defined $json_path) {
    ...
    my $report = {
        benchmark => 'benchmark-web-http1-cross-server',
...
sub print_terminal_summary ($summary) {
    my $profile = $smoke
...
sub start_server ($name, $port) {
    my $stdout_path = "/tmp/benchmark-web-$name-$$-out.log";
    my $stderr_path = "/tmp/benchmark-web-$name-$$-err.log";
    my $pid = fork();
    die "fork $name: $!\n" if !defined $pid;
