...
`server.pl` launches one Node.js process. The adapter uses only `node:http`, consumes the incoming request stream, and sends the fixed benchmark response after the request body ends.
